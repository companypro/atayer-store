...
class OrderController extends GetxController implements GetxService {
  ...
  Future<void> getOrderDetails(int orderId) async{
    var vendorId = orderRepo.getVendorId();

    Response response = await orderRepo.getOrderWithId(orderId,vendorId);
    if(response.statusCode == 200) {
  ...
  Future<void> getCurrentOrders() async {
    ...
    Response response = await orderRepo.getCurrentOrders(vendorId);
    if(response.statusCode == 200) {
      _runningOrderList = [];
      _runningOrders = [
        RunningOrderModel(status: 'pending', orderList: []),
        RunningOrderModel(status: 'processing', orderList: []),
        RunningOrderModel(status:  'prepared', orderList: []),
  ...
  // Future<void> getCurrentOrders() async {
  //   int? vendorId = orderRepo.getVendorId();
  //   if (_currentVendorId != vendorId) {
  //     _currentVendorId = vendorId;
  //     print('vendorId: $_currentVendorId');
  //     Response response = await orderRepo.getCurrentOrders(vendorId);
  //     if(response.statusCode == 200) {
  //       _runningOrderList = [];
  //       _runningOrders = [
  ...
  Future<void> getPaginatedOrders(int offset, bool reload) async {
    ...
    if (!_offsetList.contains(offset)) {
      ...
      if (response.statusCode == 200) {
        ...
        _historyOrderList!.addAll(PaginatedOrder.fromJson(response.body).orders!);
        _pageSize = PaginatedOrder.fromJson(response.body).totalSize;
        _paginate = false;
        update();
      } else {
        ApiChecker.checkApi(response);
  ...
  Future<bool> updateOrderStatus(int? orderID, String status, {bool back = false, String? reason, String? processingTime}) async {
    ...
    if (response.statusCode == 200) {
      ...
      await getCurrentOrders(); // Ensure getCurrentOrders is called after status update
      Get.find<AuthController>().getProfile();
      showCustomSnackBar(response.body['message'], isError: false);
      isSuccess = true;
    } else {
  ...
  Future<void> getOrderItemsDetails(int orderID) async {
    ...
    if(_orderModel != null && !_orderModel!.prescriptionOrder!){
      ...
      Response response = await orderRepo.getOrderDetails(orderID,vendorId);
      if(response.statusCode == 200) {
        _orderDetailsModel = [];
  ...
  // void toggleCampaignOnly() {
  //   _campaignOnly = !_campaignOnly;
  //   if (_runningOrders != null && _runningOrders!.isNotEmpty) {
  //     _runningOrders![0].orderList = [];
  //     _runningOrders![1].orderList = [];
  //     _runningOrders![2].orderList = [];
  //     _runningOrders![3].orderList = [];
